...
module.exports = {
    ...
    async execute(message, args, client) {
       ...
       if (!args[1]) return message.channel.send({embeds: [new MessageEmbed().setTitle('❌ Please speficy a duration (1day, 1hour, 1minute...)').setFooter('').setColor("ff0000")]});
       if(!member) {
           try{
            member = await message.guild.members.fetch(args[0])
               ...
               return message.channel.send({content:'Wrong ID / mention provided.'})
           }
       }
       let vipTime = timestring(args[1])
       if(!message.guild.roles.cache.has('802224235295539251')) return message.channel.send({content:'I can\'t seem to find the vip role.'});
       ...
       if(!client.vip.has(message.guild.id) || !client.vip.has(message.guild.id, member.id)){
           ...
           return message.channel.send({content:`Successfully added VIP to ${member.user.tag} for ${args[1]}`})
       }
