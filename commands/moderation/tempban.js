...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (message.member.permissions.has('BAN_MEMBERS') || message.author.id === '480692379913945099') {
            ...
            if (!argsCheck[1]) return message.channel.send({embeds: [embed.setTitle('❌ Please speficy a duration (1day, 1hour, 1minute...)').setColor("ff0000")]})
            if (!argsCheck[2]) {
            ...
            try {
                ...
                message.channel.send({embeds: [embed.setTitle('❌ Please specify a valid duration \n❌ (1day, 1hour, 1minute...)').setColor('ff0000')]})
            }
            ...
            if (user.bannable) {
                ...
                try {
                    await user.send({embeds: [embed.setFooter({text:`${client.config.bot_name} • Executed by ${message.author.username}`})
                        .setTimestamp()
                ...
                message.channel.send({embeds: [embed.setTitle('❌ I can\'t ban this user, please check my permissions').setColor('ff0000')]})
            }
        }
    }
}