...
module.exports = {
  ...
  async execute(message, args, client) {
    if (message.member.permissions.has('BAN_MEMBERS') || message.author.id === '480692379913945099') {
      let embed = new MessageEmbed().setFooter({text:`${client.config.bot_name} • Executed by ${message.author.username}`}).setTimestamp().setColor('ff0000');
      let user = args[0]
      if (user.startsWith('<@!')) return message.channel.send({embeds: [embed.setTitle("Do not mention users. Please provide their ID")]})
      tempOrPerm = []
     
        try {
       await  message.guild.members.unban(user).then(user => console.log(`Unbanned ${user.username} from ${message.guild.name}`))
          await message.channel.send({embeds: [embed.setTitle("User unbanned.").setColor('00ff00')]})
        ...
        if(client.moderation.has(message.guild.id, 'tempbannedUsers') && client.moderation.has(message.guild.id, 'tempbannedUsers.' + user))
          ...
          console.log(error)
          message.channel.send({embeds: [embed.setTitle("This user is not banned, or this ID is invalid")]})
        }
