module.exports = {
    ...
    execute(message, args, client) {
        ...
        if (client.player.getQueue(message).paused) return message.channel.send({content:`${client.emotes.error} - The music is already paused !`});

        client.player.pause(message);

        message.channel.send({content:`${client.emotes.success} - Song ${client.player.getQueue(message).playing.title} paused !`});
    },
};