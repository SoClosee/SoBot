    ...
    execute(message, args, client) {
        ...
        if (args.join(" ").toLowerCase() === 'queue') {
            if (client.player.getQueue(message).loopMode) {
                ...
                return message.channel.send({content:`${client.emotes.success} - Repeat mode **disabled** !`});
            } else {
                client.player.setLoopMode(message, true);
                return message.channel.send({content:`${client.emotes.success} - Repeat mode **enabled** the whole queue will be repeated endlessly !`});
            };
        } else {
            if (client.player.getQueue(message).repeatMode) {
                ...
                return message.channel.send({content:`${client.emotes.success} - Repeat mode **enabled** the current music will be repeated endlessly !`});
            };
        };
    },
};