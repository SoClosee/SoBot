module.exports = {
    ...
    execute(message, args, client) {
        ...
        if (!client.player.getQueue(message)) return message.channel.send({content:`${client.emotes.error} - No songs currently playing !`});

        message.channel.send({content:`**Server queue - ${message.guild.name} ${client.emotes.queue} ${client.player.getQueue(message).loopMode ? '(looped)' : ''}**\nCurrent : ${queue.playing.title} | ${queue.playing.author}\n\n` + (queue.tracks.map((track, i) => {
