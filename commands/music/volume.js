module.exports = {
    ...
    execute(message, args, client) {
        ...
        if (!args[0] || isNaN(args[0]) || args[0] === 'Infinity') return message.channel.send({content:`${client.emotes.error} - Please enter a valid number !`});

        if (Math.round(parseInt(args[0])) < 1 || Math.round(parseInt(args[0])) > 100) return message.channel.send({content:`${client.emotes.error} - Please enter a valid number (between 1 and 100) !`});

        client.player.setVolume(message, parseInt(args[0]));

        message.channel.send({content:`${client.emotes.success} - Volume set to **${parseInt(args[0])}%** !`});
    },
};